...
class ExceptionHelper {
  /// Convertir DioException a ApiException usando el nuevo formato del backend
  static ApiException fromDioException(DioException e, {String? defaultMessage}) {
    final responseData = e.response?.data;
    final statusCode = e.response?.statusCode;

    // Si hay respuesta con datos, intentar parsear el nuevo formato
    if (responseData is Map<String, dynamic>) {
      try {
        return ApiException.fromResponse(responseData, statusCode: statusCode);
      } catch (_) {
        // Si falla el parsing, continuar con el método tradicional
      }
    }
    ...
    switch (statusCode) {
      ...
      case 422:
        ...
        return ApiException(
          validationMessage ?? 'Error de validación',
          statusCode: statusCode,
          errors: errors,
      ...
      default:
        return ApiException(
          message ?? defaultMessage ?? 'Error desconocido: ${e.message ?? e.type.toString()}',
          statusCode: statusCode,
          errors: errors,
        );
    }
