...
class ClientService {
  ...
  static Future<PaginatedResponse<ClientModel>> getClients({
    ...
    try {
      ...
      return PaginatedResponse.fromJson(
        responseData,
        (json) {
          // Si el json ya es un objeto cliente, parsearlo directamente
  ...
  static Future<ClientModel> createClient(ClientModel client) async {
    try {
      ...
      if (e.response?.statusCode == 422) {
        ...
        // Priorizar mensaje de error específico del campo, luego mensaje general de la API
  ...
  static Future<void> deleteClient(int id) async {
    try {
      await ApiService.delete('/datero/clients/$id');
    } on DioException catch (e) {
      final responseData = e.response?.data;
      String? apiMessage;
      if (responseData is Map<String, dynamic>) {
  ...
  static Future<ClientOptions> getOptions() async {
    try {
      final response = await ApiService.get('/datero/clients/options');
      final responseData = response.data as Map<String, dynamic>;
